(function ($) {
  ...
  if ($('#clock').length) {
    ...
    $('#clock').countdown('2999/03/29 00:00:00', function (event) {
      const loveDate = new Date('2013/03/29 00:00:00').getTime();
      const currentTime = new Date().getTime();
      let loveTime = currentTime - loveDate;

      const countDate = Math.ceil(loveTime / (1000 * 60 * 60 * 24)) - 1;

      loveTime = loveTime % (1000 * 60 * 60 * 24);

      const countHours = Math.ceil(loveTime / (1000 * 60 * 60)) - 1;

      loveTime = loveTime % (1000 * 60 * 60);

      const countMins = Math.ceil(loveTime / (1000 * 60)) - 1;

      loveTime = loveTime % (1000 * 60);

      const countSecs = Math.ceil(loveTime / 1000) - 1;

      // console.log(daysLove);
      // console.log(loveTime);
      var $this = $(this).html(
        event.strftime(
          '' +
            `<div class="box"><div>${countDate}</div> <span>Ngày</span> </div>` +
            `<div class="box"><div>${countHours}</div> <span>Giờ</span> </div>` +
            `<div class="box"><div>${countMins}</div> <span>Phút</span> </div>` +
            `<div class="box"><div>${countSecs}</div> <span>Giây</span> </div>`
        )
      );
    });
  }
  ...
  function countDayHourMinuteSecond() {
    var loveDate = new Date(2012, 3, 29);
    var dateNow = new Date();

    var seconds = Math.floor((dateNow - loveDate) / 1000);
